...
//
// nav_msgs::Odometry getDiffOdom(nav_msgs::Odometry & new, nav_msgs::Odometry &old)
// {
//   nav_msgs::Odometry diff = new;
//   diff.
//   return ;
// }
  ...
  if ( ( msg_flags & HAS_POS ) && ( msg_flags & HAS_Q ) && ( msg_flags & HAS_W ) && ( msg_flags & HAS_V ) )
    {
      ...
      //last_odometry =last_odometry-last_odometry ;
      ...
      if ( 0 )//Abandon this
        {
          ...
          // set
          odom_trans.transform.translation.x = local_position.x;
          odom_trans.transform.translation.y = local_position.y;
      ...
      // send
      else //( start_searching_flag)
        {
          ...
          // set
          odom_trans.transform.translation.x = local_position.x-last_odometry.pose.pose.position.x;
          odom_trans.transform.translation.y = local_position.y-last_odometry.pose.pose.position.y;
          odom_trans.transform.translation.z = 0.0;
          odom_trans.transform.rotation = odom_quat;
          ...
          odometry.child_frame_id = "base_link";
          odometry.header.stamp = current_time;
          odometry.pose.pose.position.x = local_position.x-last_odometry.pose.pose.position.x;
          odometry.pose.pose.position.y = local_position.y-last_odometry.pose.pose.position.y;
          odometry.pose.pose.position.z = 0.0;
          odometry.pose.pose.orientation.w = attitude_quaternion.q0;
          odometry.pose.pose.orientation.x = attitude_quaternion.q1;
          odometry.pose.pose.orientation.y = attitude_quaternion.q2;

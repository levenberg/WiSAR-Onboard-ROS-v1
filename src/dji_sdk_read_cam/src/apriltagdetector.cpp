...
//          << " (Hamming: " << detection.hammingDistance << ")";
  ...
  dji_sdk_read_cam::AprilTagPose calculatedPose;
    calculatedPose.header.frame_id = "x3_pose_wrt_apriltag";
...
// float last_flight_height = 0.0;
  ...
  if ( detections.empty() ) // no Tag found
    {
        calculatedPose.header.stamp = ros::Time::now();
        calculatedPose.x = 0;
        calculatedPose.y = 0;
        calculatedPose.z = 0;
        calculatedPose.norm = 0.0;
        calculatedPose.gimbal_pitch_inc = 0.0;
        calculatedPose.istracked = false;
      ...
      m_result_pub.publish ( calculatedPose );
    }
  ...
  for ( unsigned int i=0; i<detections.size(); i++ )
    {
      ...
      if ( m_isShowResult )
        ...
        calculatedPose.header.stamp = ros::Time::now();
        calculatedPose.x = translation ( 0 );
        calculatedPose.y = translation ( 1 );

        calculatedPose.yaw = yaw;
        calculatedPose.pitch = pitch;
        calculatedPose.roll = roll;
        calculatedPose.norm = translation.norm();
        calculatedPose.gimbal_pitch_inc = atan ( translation ( 2 ) /translation ( 0 ) ) *57.2958;
        calculatedPose.istracked = true;

#ifndef GIMBAL_USED
      calculatedPose.height_with_gimbal = translation ( 0 );
      calculatedPose.z = translation ( 2 );//IMPORTANT NOTE: Uisng GIMBAL makes it a little bit different
#else //NOTE: The following only makes sense when GIMBAL CONTROLLING is USED.
      if ( m_gimbal.pitch+90.0<0.1 )
        {
            calculatedPose.z = translation ( 2 );
      ...
      else
        {
          ...
          float temp = sqrt ( pow ( translation ( 0 ),2 ) +pow ( translation ( 2 ),2 ) );
            calculatedPose.z = temp *sin ( 0.017453* ( calculatedPose.gimbal_pitch_inc+m_gimbal.pitch+90.0 ) ); // /57.2958 ); //IMPORTANT NOTE: Uisng GIMBAL makes it a little bit different
            calculatedPose.height_with_gimbal = temp*cos ( ( calculatedPose.gimbal_pitch_inc+m_gimbal.pitch+90.0 ) /57.2958 );
...
#endif
      calculatedPose.gimbal_yaw_inc = atan(-calculatedPose.y/(EPS+calculatedPose.z))* 57.2958;
